...
$sql = "SELECT DISTINCT subject FROM questions_lop2 ORDER BY subject ASC";
...
<html lang="vi">
    ...
    if ($subjects->num_rows > 0) {
        while ($sub = $subjects->fetch_assoc()) {
            ...
            $sql2 = "SELECT DISTINCT lesson FROM questions_lop2 WHERE subject='$subject' ORDER BY lesson ASC";
            $lessons = $conn->query($sql2);

            if ($lessons->num_rows > 0) {
                while ($les = $lessons->fetch_assoc()) {
                    $lesson = $les['lesson'];                           //tên DB
                    echo "<a class='lesson-link' href='Phan_KT_lop2/lay_DB_lop2.php?subject=" . urlencode($subject) . "&lesson=$lesson'><p>📖 Bài kiểm tra tuần $lesson</p></a>";
                }
            } else {
